...
const usuariosController = {
  ...
  crearUser: async (req, res) => {
    try {
      ...
      if (roles && roles.length > 0) {
        ...
        const inserts = rolesEncontrados.map((r) => ({
      ...
      if (
        usuarioConRoles &&
        usuarioConRoles.tb_rol &&
        Array.isArray(usuarioConRoles.tb_rol)
      ) {
        const seen = new Set();
  ...
  actualizarUser: async (req, res) => {
    try {
      ...
      if (
        usuarioActualizado &&
        usuarioActualizado.tb_rol &&
        Array.isArray(usuarioActualizado.tb_rol)
      ) {
        const seen = new Set();
