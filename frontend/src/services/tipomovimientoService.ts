...
export const tipomovimientoService = {
  ...
  async createTipoMovimiento(
    data: CreateTipoMovimientoRequest,
  ): Promise<TipoMovimientoItem> {
    const response = await api.post<ApiResponse<TipoMovimientoItem>>(
      "/tipomovimiento",
      data,
    );
  ...
  async updateTipoMovimiento(
    id: number,
    data: Partial<CreateTipoMovimientoRequest>,
  ): Promise<TipoMovimientoItem> {
    const response = await api.put<ApiResponse<TipoMovimientoItem>>(
      `/tipomovimiento/${id}`,
      data,
    );
    return response.data.data;
