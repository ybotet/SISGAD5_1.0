...
export const trabajadorService = {
  ...
  async updateTrabajador(
    id: number,
    data: Partial<CreateTrabajadorRequest>,
  ): Promise<TrabajadorItem> {
    const response = await api.put<ApiResponse<TrabajadorItem>>(`/trabajador/${id}`, data);
    return response.data.data;
  },

  async getTrabajador(id: number): Promise<TrabajadorItem> {
    const response = await api.get<ApiResponse<TrabajadorItem>>(`/trabajador/${id}`);
    return response.data.data;
  },
