import type { SenalizacionItem } from '../../services/senalizacionService';

interface SenalizacionModalProps {
    show: boolean;
    editingItem: SenalizacionItem | null;
    saving: boolean;
    onClose: () => void;
    onSave: (formData: FormData) => void;
}

export default function SenalizacionModal({
    show,
    editingItem,
    saving,
    onClose,
    onSave
}: SenalizacionModalProps) {
    if (!show) return null;
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 w-full max-w-md mx-4">
                <h3 className="text-lg font-semibold mb-4">
                    {editingItem ? 'Editar Señalización' : 'Nueva Señalización'}
                </h3>
                    ...
                    <div className="space-y-4">
                        <div>
                            ...
                            <input
                                type="text"
                                name="senalizacion"
                                defaultValue={editingItem?.senalizacion || ''}
                                required
                                maxLength={18}
                                disabled={saving}
                            ...
                            />
                            <p className="text-xs text-gray-500 mt-1">Máximo 18 caracteres</p>
                        </div>
                    </div>
