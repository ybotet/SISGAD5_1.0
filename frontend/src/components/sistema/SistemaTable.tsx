import type { SistemaItem } from '../../services/sistemaService';

interface SistemaTableProps {
    items: SistemaItem[];
    searchTerm: string;
    onEdit: (item: SistemaItem) => void;
                        ...
                        {items.length === 0 ? (
                            ...
                            items.map((item) => (
                                <tr key={item.id_sistema} className="hover:bg-gray-50">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                        {item.tb_propietario?.nombre || 'Sin propietario'}
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div className="flex justify-end space-x-2">
                                            ...
                                            <button
                                                onClick={() => onDelete(item.id_sistema)}
