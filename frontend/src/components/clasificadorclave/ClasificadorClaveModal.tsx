...
interface ClasificadorClaveModalProps {
    show: boolean;
    editingItem: ClasificadorclaveItem | null;
    saving: boolean;
    onClose: () => void;
    onSave: (formData: FormData) => void;
}

export default function ClasificadorClaveModal({
    show,
    editingItem,
    saving,
    onClose,
    onSave
}: ClasificadorClaveModalProps) {
    if (!show) return null;
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 w-full max-w-md mx-4">
                <h3 className="text-lg font-semibold mb-4">
                    {editingItem ? 'Editar Clasificador de Clave' : 'Nuevo Clasificador de Clave'}
                </h3>
                    ...
                    <div className="space-y-4">
                        <div>
                            ...
                            <input
                                type="text"
                                name="clasificador"
                                defaultValue={editingItem?.clasificador || ''}
                                required
                                disabled={saving}
