...
import QuejaFlowHistorySection from "./QuejaFlowHistorySection";
...
export default function QuejaDetallesModal({
  ...
  const [eliminandoTrabajo, setEliminandoTrabajo] = useState<number | null>(
    null,
  );
  ...
  // Establecer fecha actual cuando se abre formulario de nueva prueba
  useEffect(() => {
    if (showNuevaPrueba) {
      setNuevaPrueba((prev) => ({
        ...prev,
  ...
  // Establecer fecha actual cuando se abre formulario de nuevo trabajo
  useEffect(() => {
    if (showNuevoTrabajo) {
  ...
  const handleCrearPrueba = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await quejaService.createPrueba({
        ...
        id_resultado: nuevaPrueba.id_resultado
          ? parseInt(nuevaPrueba.id_resultado)
          : null,
        id_trabajador: nuevaPrueba.id_trabajador
          ? parseInt(nuevaPrueba.id_trabajador)
          : null,
          ...
          <div className="p-6">
            {loading ? (
              ...
              <div className="text-center py-8">
                ...
                <p className="text-gray-600">
                  No se pudieron cargar los detalles de la queja
                </p>
              </div>
            ) : (
              <>
                ...
                <QuejaFlowHistorySection
                  flujo={flujo}
                  clavesMap={new Map(claves.map((c) => [c.id_clave, c.clave]))}
                />
                ...
                <div className="mb-8">
                  <div className="flex justify-between items-center mb-4">
                    ...
                    >
                      <i
                        className={
                          showNuevaPrueba ? "ri-close-line" : "ri-add-line"
                        }
                      ></i>
                      <span>
                        {showNuevaPrueba ? "Cancelar" : "Nueva Prueba"}
                      </span>
                    </button>
                  </div>
                        ...
                        {pruebas.length === 0 ? (
                          ...
                          pruebas.map((prueba) => (
                            <tr key={prueba.id_prueba}>
                              ...
                              <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-900">
                                {prueba.tb_trabajador
                                  ? prueba.tb_trabajador.clave_trabajador
                                  : "N/A"}
                              </td>
                              <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-900">
                              ...
                              <td className="px-4 py-4 whitespace-nowrap text-sm">
                                <button
                                  onClick={() =>
                                    handleEliminarPrueba(prueba.id_prueba)
                                  }
                                  disabled={
                ...
                <div>
                  <div className="flex justify-between items-center mb-4">
                    ...
                    >
                      <i
                        className={
                          showNuevoTrabajo ? "ri-close-line" : "ri-add-line"
                        }
                      ></i>
                      <span>
                        {showNuevoTrabajo ? "Cancelar" : "Nuevo Trabajo"}
                      </span>
                    </button>
                  </div>
                        ...
                        {trabajos.length === 0 ? (
                          ...
                          trabajos.map((trabajo) => (
                            <tr key={trabajo.id_trabajo}>
                              ...
                              <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-900">
                                {(() => {
                                  if (!trabajo.estado) return "N/A";
                                  const idNum =
                                    typeof trabajo.estado === "string"
                                      ? parseInt(trabajo.estado)
                                      : trabajo.estado;
                                  const found = claves.find(
                                    (c) => c.id_clave === idNum,
                                  );
                                  return found
                                    ? found.clave
                              ...
                              <td className="px-4 py-4 whitespace-nowrap text-sm">
                                <button
                                  onClick={() =>
                                    handleEliminarTrabajo(trabajo.id_trabajo)
                                  }
                                  disabled={
