...
interface CategoriaMaterialModalProps {
  showModal: boolean;
  editingItem: CategoriaMaterialItem | null;
  saving: boolean;
  onClose: () => void;
  onSave: (formData: FormData) => void;
}

export default function CategoriaMaterialModal({
  showModal,
  editingItem,
  saving,
  onClose,
  onSave,
}: CategoriaMaterialModalProps) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 w-full max-w-md mx-4">
        <h3 className="text-lg font-semibold mb-4">
          {editingItem ? "Editar categoría de material" : "Nueva categoría de material"}
        </h3>
