...
interface GruposTrabajoTableProps {
    items: GrupoTrabajoItem[];
    onEdit: (item: GrupoTrabajoItem) => void;
    onDelete: (id: number) => void;
    loading?: boolean;
}

export default function GruposTrabajoTable({
    items,
    onEdit,
    onDelete,
    loading = false
}: GruposTrabajoTableProps) {
    if (loading) {
        return (
                                ...
                                <tr key={item.id_grupow} className="hover:bg-gray-50">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div className="flex justify-end space-x-2">
                                            ...
                                            <button
                                                onClick={() => onDelete(item.id_grupow)}
