...
interface GruposTrabajoModalProps {
    show: boolean;
    editingItem: GrupoTrabajoItem | null;
    saving: boolean;
    onClose: () => void;
    onSave: (formData: FormData) => void;
}

export default function GruposTrabajoModal({
    show,
    editingItem,
    saving,
    onClose,
    onSave
}: GruposTrabajoModalProps) {
    if (!show) return null;
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 w-full max-w-md mx-4">
                <h3 className="text-lg font-semibold mb-4">
                    {editingItem ? 'Editar Grupo de Trabajo' : 'Nuevo Grupo de Trabajo'}
                </h3>
                    ...
                    <div className="space-y-4">
                        <div>
                            ...
                            <input
                                type="text"
                                name="grupo"
                                defaultValue={editingItem?.grupo || ''}
                                required
                                disabled={saving}
