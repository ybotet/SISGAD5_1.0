...
interface TipolineaModalProps {
    show: boolean;
    editingItem: TipolineaItem | null;
    saving: boolean;
    onClose: () => void;
    onSave: (formData: FormData) => void;
}

export default function TipolineaModal({
    show,
    editingItem,
    saving,
    onClose,
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 w-full max-w-md mx-4">
                <h3 className="text-lg font-semibold mb-4">
                    {editingItem ? 'Editar Tipo de Línea' : 'Nuevo Tipo de Línea'}
                </h3>
                <form onSubmit={handleSubmit}>
                    <div className="space-y-4">
                        <div>
                            ...
                            />
                            <p className="text-xs text-gray-500 mt-1">Máximo 23 caracteres</p>
                        </div>
                    </div>
