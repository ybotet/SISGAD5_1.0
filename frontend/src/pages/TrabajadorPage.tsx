...
export default function TrabajadorPage() {
  ...
  const loadGrupos = async () => {
    try {
      const response = await grupowService.getGruposTrabajo(1, 100, "");
  ...
  const loadTrabajadores = async (page: number = 1, limit: number = 10, search: string = "") => {
    try {
      setLoading(true);
      setError("");
      const response: PaginatedResponse<TrabajadorItem> = await trabajadorService.getTrabajadores(
        page,
        limit,
        search,
      );
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      const errorMessage =
        err?.response?.data?.error || err?.message || t("trabajador.errorDelete");
  ...
  const handleSave = async (formData: FormData) => {
    try {
      ...
      const errorMessage =
        ...
        err?.message ||
        (editingItem ? "Error al actualizar el trabajador" : "Error al crear el trabajador");
      setError(errorMessage);
    } finally {
      setSaving(false);
    }
  ...
  return (
    <div className="p-6">
      ...
      <TrabajadorError error={error} onClose={() => setError("")} />
