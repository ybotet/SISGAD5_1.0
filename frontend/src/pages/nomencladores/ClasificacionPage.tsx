...
export default function ClasificacionPage({ type }: ClasificacionPageProps) {
  ...
  const loadClasificaciones = async (page: number = 1, limit: number = 10) => {
    try {
      setLoading(true);
      setError("");
      const response: PaginatedResponse<ClasificacionItem> =
        await clasificacionService.getClasificaciones(type, page, limit);
  ...
  const handleSave = async (formData: FormData) => {
    try {
      ...
      setError(
        editingItem ? "Error al actualizar la clasificación" : "Error al crear la clasificación",
      );
  ...
  return (
    <div className="p-6">
      ...
      {/* Error */}
      <ClasificacionError error={error} onClose={() => setError("")} />
