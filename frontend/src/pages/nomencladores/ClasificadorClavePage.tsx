...
export default function ClasificadorClavePage() {
  ...
  const loadClasificadoresClave = async (
    page: number = 1,
    limit: number = 10,
    search: string = "",
  ) => {
    try {
      setLoading(true);
      setError("");
      const response: PaginatedResponse<ClasificadorclaveItem> =
        await clasificadorclaveService.getClasificadoresClave(page, limit, search);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      const errorMessage =
        err?.response?.data?.error || err?.message || "Error al eliminar el clasificador de clave";
  ...
  const handleSave = async (formData: FormData) => {
    try {
      ...
      if (editingItem) {
        await clasificadorclaveService.updateClasificadorclave(
          editingItem.id_clasificadorclave,
          itemData,
        );
      } else {
      ...
      const errorMessage =
        ...
        err?.message ||
        (editingItem
          ? "Error al actualizar el clasificador de clave"
          : "Error al crear el clasificador de clave");
      setError(errorMessage);
  ...
  return (
    <div className="p-6">
      ...
      {/* Error */}
      <ClasificadorClaveError error={error} onClose={() => setError("")} />
