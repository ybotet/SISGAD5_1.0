...
export default function SenalizacionPage() {
  ...
  const loadSenalizacion = async (page: number = 1, limit: number = 10, search: string = "") => {
    try {
      setLoading(true);
      setError("");
      const response: PaginatedResponse<SenalizacionItem> =
        await senalizacionService.getSenalizacion(page, limit, search);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      const errorMessage =
        err?.response?.data?.error || err?.message || "Error al eliminar el senalizacion";
  ...
  return (
    <div className="p-6">
      ...
      {/* Error */}
      <SenalizacionError error={error} onClose={() => setError("")} />
