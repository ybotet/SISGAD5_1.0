...
export default function TipoMovimientoPage() {
  ...
  const loadTiposMovimiento = async (page: number = 1, limit: number = 10, search: string = "") => {
    try {
      setLoading(true);
      setError("");
      const response: PaginatedResponse<TipoMovimientoItem> =
        await tipomovimientoService.getTiposMovimiento(page, limit, search);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      const errorMessage =
        err?.response?.data?.error || err?.message || "Error al eliminar el tipo de movimiento";
  ...
  return (
    <div className="p-6">
      ...
      {/* Error */}
      <TipoMovimientoError error={error} onClose={() => setError("")} />
