...
export default function TipoPizarraPage() {
  ...
  const loadTiposPizarra = async (page: number = 1, limit: number = 10, search: string = "") => {
    try {
      setLoading(true);
      setError("");
      const response: PaginatedResponse<TipoPizarraItem> = await tipopizarraService.getTiposPizarra(
        page,
        limit,
        search,
      );
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      const errorMessage =
        err?.response?.data?.error || err?.message || "Error al eliminar el tipo de pizarra";
  ...
  const handleSave = async (formData: FormData) => {
    try {
      ...
      const errorMessage =
        ...
        err?.message ||
        (editingItem
          ? "Error al actualizar el tipo de pizarra"
          : "Error al crear el tipo de pizarra");
      setError(errorMessage);
  ...
  return (
    <div className="p-6">
      ...
      {/* Error */}
      <TipoPizarraError error={error} onClose={() => setError("")} />
