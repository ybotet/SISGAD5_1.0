...
export default function TipoQuejaPage() {
  ...
  const loadTiposQueja = async (page: number = 1, limit: number = 10, search: string = "") => {
    try {
      setLoading(true);
      setError("");
      const response: PaginatedResponse<TipoQuejaItem> = await tipoquejaService.getTiposQueja(
        page,
        limit,
        search,
      );
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      const errorMessage =
        err?.response?.data?.error || err?.message || "Error al eliminar el tipo de queja";
  ...
  return (
    <div className="p-6">
      ...
      {/* Error */}
      <TipoQuejaError error={error} onClose={() => setError("")} />
