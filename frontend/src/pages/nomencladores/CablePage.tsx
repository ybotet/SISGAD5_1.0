...
import type { CableItem, CreateCableRequest, PaginatedResponse } from "../../services/cableService";
...
export default function CablePage() {
  ...
  const loadCables = async (page: number = 1, limit: number = 10, search: string = "") => {
    try {
      setLoading(true);
      setError("");
      const response: PaginatedResponse<CableItem> = await cableService.getCables(
        page,
        limit,
        search,
      );
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      const errorMessage =
        err?.response?.data?.error || err?.message || "Error al eliminar el cable";
  ...
  return (
    <div className="p-6">
      ...
      {/* Error */}
      <CableError error={error} onClose={() => setError("")} />
