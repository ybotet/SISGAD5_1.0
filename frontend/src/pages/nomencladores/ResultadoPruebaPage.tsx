...
export default function ResultadoPruebaPage() {
  ...
  const loadResultadoPruebaes = async (
    page: number = 1,
    limit: number = 10,
    search: string = "",
  ) => {
    try {
      setLoading(true);
      setError("");
      const response: PaginatedResponse<ResultadoPruebaItem> =
        await resultadopruebaService.getResultadoPrueba(page, limit, search);
  ...
  const handleSave = async (formData: FormData) => {
    try {
      ...
      if (editingItem) {
        await resultadopruebaService.updateResultadoPrueba(
          editingItem.id_resultadoprueba,
          itemData,
        );
      } else {
      ...
      setError(
        editingItem
          ? "Error al actualizar la resultado de pruebas"
          : "Error al crear la resultado de pruebas",
      );
  ...
  return (
    <div className="p-6">
      ...
      {/* Error */}
      <ResultadoPruebaError error={error} onClose={() => setError("")} />
