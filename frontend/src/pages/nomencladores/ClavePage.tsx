...
import type { ClaveItem, CreateClaveRequest, PaginatedResponse } from "../../services/claveService";
...
export default function ClavePage() {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      const errorMessage =
        err?.response?.data?.error || err?.message || "Error al eliminar la clave";
  ...
  const handleSave = async (datosEnvio: any) => {
    try {
      setSaving(true);
      setError("");
      ...
      const itemData: CreateClaveRequest = {
        ...
        valor_p: datosEnvio.valor_p, // Ya viene como número del modal
        es_pendiente: datosEnvio.es_pendiente, // Ya viene como booleano
      };

      console.log("📝 Enviando al backend:", itemData);
  ...
  return (
    <div className="p-6">
      ...
      {/* Error */}
      <ClaveError error={error} onClose={() => setError("")} />
