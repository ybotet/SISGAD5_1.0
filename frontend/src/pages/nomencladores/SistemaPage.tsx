...
import type { SistemaItem, PaginatedResponse, Propietario } from "../../services/sistemaService";
...
export default function SistemaPage() {
  ...
  const loadSistema = async (page: number = 1, limit: number = 10, search: string = "") => {
    try {
      setLoading(true);
      setError("");
      const response: PaginatedResponse<SistemaItem> = await sistemaService.getSistema(
        page,
        limit,
        search,
      );
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      const errorMessage =
        err?.response?.data?.error || err?.message || "Error al eliminar el sistema";
  ...
  return (
    <div className="p-6">
      ...
      {/* Error */}
      <SistemaError error={error} onClose={() => setError("")} />
