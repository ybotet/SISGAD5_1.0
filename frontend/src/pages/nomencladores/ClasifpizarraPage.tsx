...
export default function ClasifpizarraPage() {
  ...
  const loadClasificacionesPizarra = async (
    page: number = 1,
    limit: number = 10,
    search: string = "",
  ) => {
    try {
      setLoading(true);
      setError("");
      const response: PaginatedResponse<ClasifpizarraItem> =
        await clasifpizarraService.getClasificacionesPizarra(page, limit, search);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      const errorMessage =
        ...
        err?.message ||
        "Error al eliminar la clasificación de pizarra";
  ...
  const handleSave = async (formData: FormData) => {
    try {
      ...
      const errorMessage =
        ...
        err?.message ||
        (editingItem
          ? "Error al actualizar la clasificación de pizarra"
          : "Error al crear la clasificación de pizarra");
      setError(errorMessage);
  ...
  return (
    <div className="p-6">
      ...
      {/* Error */}
      <ClasifpizarraError error={error} onClose={() => setError("")} />
