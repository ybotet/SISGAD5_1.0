...
export default function UsuariosPage() {
  ...
  const [showMultipleConfirmModal, setShowMultipleConfirmModal] =
    useState(false);
  ...
  const loadUsuarios = async (
    ...
    try {
      ...
      const response: PaginatedResponse<Usuario> =
        await usuariosService.getUsuarios(page, limit, search);
  ...
  const handleConfirmDeleteMultiple = async () => {
    ...
    try {
      ...
      setItems((prev) =>
        prev.filter((user) => !selectedIds.includes(user.id_usuario)),
      );
      ...
      setError(
        `✓ Se eliminaron ${result.eliminados} de ${result.totalSolicitados} usuario(s) correctamente`,
      );
      setTimeout(() => setError(""), 3000);
    } catch (err: any) {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      // Actualizar la lista local
      setItems((prev) =>
        prev.filter((user) => user.id_usuario !== itemToDelete),
      );
  ...
  const handleSave = async (formData: FormData) => {
    try {
      ...
      const roles = formData
        .getAll("roles")
        .map((role) => parseInt(role as string));
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <UsuariosConfirmModal
        ...
        confirmText={
          deletingMultiple
            ? "Eliminando..."
            : `Sí, eliminar ${selectedIds.length} usuario(s)`
        }
        cancelText="Cancelar"
