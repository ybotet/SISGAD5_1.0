...
app.use('/api', createProxyMiddleware({
    target: process.env.SERVICES_SERVICE_URL || 'http://localhost:5002',
    changeOrigin: true
}));
...
app.get('/', (req, res) => {
    res.json({
        ...
        services: {
            ...
            services: process.env.SERVICES_SERVICE_URL || 'http://localhost:5002'
        }
    });
});
...
app.listen(PORT, () => {
    ...
    console.log(`👥 Servicio Usuarios: ${process.env.USERS_SERVICE_URL || 'http://localhost:5001'}`);
    console.log(`🛠️ Servicio Servicios: ${process.env.SERVICES_SERVICE_URL || 'http://localhost:5002'}`);
