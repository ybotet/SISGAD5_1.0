...
const UserRolesController = {
  ...
  async getAll(req, res) {
    try {
      ...
      const data = await UserRoles.findAndCountAll({
        where: whereClause,
  ...
  async getById(req, res) {
    try {
      const { id } = req.params;
      const data = await UserRoles.findByPk(id);
  ...
  async update(req, res) {
    try {
      ...
      const [affectedRows] = await UserRoles.update(req.body, {
        where: { id_user_roles: id }
      });
      ...
      const updatedData = await UserRoles.findByPk(id);
  ...
  async delete(req, res) {
    try {
      ...
      const affectedRows = await UserRoles.destroy({
        where: { id_user_roles: id }
      });
